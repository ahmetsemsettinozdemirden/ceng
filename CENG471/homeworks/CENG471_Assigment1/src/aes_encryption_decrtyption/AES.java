package aes_encryption_decrtyption;

import static aes_encryption_decrtyption.AESUtil.GMul;
    ...
    private static void mixColumns(byte[] state, byte[][] MDS) {
        ...
        for (int col = 0; col < 4; col++) {
            for (int row = 0; row < 4; row++) {
                ...
                newState[(col * 4) + row] = (byte) (GMul(MDS[row][0], state[(col * 4)]) ^
                                                    GMul(MDS[row][1], state[(col * 4) + 1]) ^
                                                    GMul(MDS[row][2], state[(col * 4) + 2]) ^
                                                    GMul(MDS[row][3], state[(col * 4) + 3]));
            }
        }
